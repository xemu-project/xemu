...
void pgraph_gl_init_display(NV2AState *d)
{
    ...
    const char *fs =
        ...
        "                           greaterThan(screenCoord, "
        "output_region.zw));\n"
        "        if (!any(clip) && (!pvideo_color_key_enable || out_Color.rgba "
        "== pvideo_color_key)) {\n"
        "            vec2 out_xy = (screenCoord - pvideo_pos.xy) * "
        "pvideo_scale.z;\n"
        "            vec2 in_st = (pvideo_in_pos + out_xy * pvideo_scale.xy) / "
        "textureSize(pvideo_tex, 0);\n"
        "            in_st.y *= -1.0;\n"
...
void pgraph_gl_download_overlapping_surfaces(NV2AState *d, hwaddr start,
                                             hwaddr end)
{
    ...
    QTAILQ_FOREACH (surface, &d->pgraph.gl_renderer_state->surfaces, entry) {
        ...
        bool overlapping =
            !(surface->vram_addr >= end || start >= surf_vram_end);
        if (overlapping) {
...
static bool check_framebuffer_dirty(NV2AState *d, hwaddr framebuffer,
                                    hwaddr framebuffer_end)
{
    ...
    return memory_region_test_and_clear_dirty(
        d->vram, framebuffer, framebuffer_end - framebuffer, DIRTY_MEMORY_VGA);
}
...
static void render_display(NV2AState *d, SurfaceBinding *surface)
{
    ...
    if (surface && surface->color && surface->width == width &&
        ...
        pgraph_gl_download_overlapping_surfaces(d, framebuffer,
                                                framebuffer_end);
