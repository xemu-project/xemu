...
void MainMenuInputView::Draw()
{
    ...
    if (bound_state) {
        ...
        if (hasFirmware) {
            ...
            static const SDL_DialogFileFilter firmware_filters[] = {
                { ".bin Files", "bin" },
                { "All Files", "*" }
            };
            ...
            FilePicker("DVD Kit Firmware", firmware_path, firmware_filters, 2, false,
                [](const char *path) {
                    xemu_save_dvd_firmware_settings(active, path);
                });
